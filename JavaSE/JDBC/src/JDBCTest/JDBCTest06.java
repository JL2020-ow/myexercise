 ...
 * 1、需求：模拟用户登录功能的实现
 * 2、业务描述：
 * 程序运行的时候，提供一个输入的的入口，可以让用户输入用户名和密码。
 * 用户输入用户名和密码之后，提交信息，java程序收集到用户信息。
 * java程序连接数据库验证用户名和密码是否合法。 合法：显示登录成功 不合法：显示登录失败
 * 3、数据的准备 在实际开发中，表的设计会使用专业的建模工具——powerdesigner 使用PD工具来进行数据库表的设计。
 * 4、当前程序存在问题：输入用户名密码错误的情况下也能登录成功。
 *      用户名：fdsa
 *      密码：fdsa' or '1'=1
 *     这种现象被成为“sql注入”。
 * 5、导致sql注入的根本原因是什么？ line 60 61
 *     用户输入的信息中含有sql语句的关键字，并且这些关键字参与sql语句的编译过程。
 *     导致sql语句的院意被扭曲，进而达到sql注入。
 */
